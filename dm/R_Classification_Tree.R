...
model = glm(formula = Target ~ .,, family = binomial, data = CTDF.dev[,-1])
summary(model)


printcp(m1) #	display cp table
plotcp(m1) 	#plot cross-validation results
rsq.rpart(m1) #	plot approximate R-squared and relative error for different splits (2 plots). labels are only appropriate for the "anova" method.
print(m1) 	#print results

summary(m1) #	detailed results including surrogate splits
plot(m1) 	#plot decision tree
text(m1) #	label the decision tree plot
post(m1, file=) 	


m2 <- rpart(formula = Target ~ Holding_Period + No_OF_CR_TXNS, data = CTDF.dev[,-1], method = "class", control = r.ctrl)
m2
